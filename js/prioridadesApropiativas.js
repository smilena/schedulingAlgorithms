...
$("#generarTabla").bind("click", function(){
	...
	for(var i = 1;i<=numeroProcesos;i++){
		...
		$("#datos tbody").append("<tr><td style='color:"+colorTemp+";'>"+i+"</td><td><span>"+tiempos[i-1]+"</span></td><td>"+prioridades[i-1]+"</td></tr>");
...
var ordenarProcesos = function(){
	for(var i=0;i<numeroProcesos-1;i++){
		for(var j=i+1;j<numeroProcesos;j++){
			if(espera[i].prioridad>espera[j].prioridad){
				var temp=espera[i];
				espera[i]=espera[j];
				espera[j]=temp;
			}
...
$("#simular").bind("click",function(){
	...
	$.each(espera,function(index,proceso){				
		$("#espera").append("<div class='span12' id='P"+proceso.id+"' style='height:20px; background-color:"+proceso.color+"; float:left; text-align:center;'><span class='pequeno'>Id "+proceso.id+" T "+proceso.tiempo+" Pri "+proceso.prioridad+"</span></div>");		
	});
